...
public class JanelaFormProd extends JDialog {
	...
	private JCheckBox chckbxAtiva = new JCheckBox("ATIVA");
	private Produtora objSaida = new Produtora();
	private JTextField txtProdutora;
	
	public Produtora getObjSaida() {
		return objSaida;
	}
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						objSaida.setProNome(txtProdutora.getText());
						if (chckbxAtiva.isSelected()){
							objSaida.setProSituacao(1);
						}
						else{
							objSaida.setProSituacao(0);
						}
						dispose();
					}
				});
	...
	public JanelaFormProd(Integer IdItem) {
		...
		{
			JLabel lblID = new JLabel(alteraItem.getProId().toString());
			lblID.setAlignmentX(Component.CENTER_ALIGNMENT);
			GridBagConstraints gbc_lblID = new GridBagConstraints();
			gbc_lblID.fill = GridBagConstraints.BOTH;
		...
		{
			txtProdutora = new JTextField(alteraItem.getProNome());
			txtProdutora.setAlignmentX(Component.RIGHT_ALIGNMENT);
			GridBagConstraints gbc_txtProdutora = new GridBagConstraints();
			gbc_txtProdutora.fill = GridBagConstraints.BOTH;
			gbc_txtProdutora.insets = new Insets(0, 0, 5, 0);
		...
		{
			if (alteraItem.getProSituacao() == 1){
				chckbxAtiva.setSelected(true);
			}
			else{
				chckbxAtiva.setSelected(false);
			}
			GridBagConstraints gbc_chckbxAtiva = new GridBagConstraints();
			gbc_chckbxAtiva.fill = GridBagConstraints.BOTH;
			gbc_chckbxAtiva.gridx = 1;
		...
		}
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						objSaida.setProId(alteraItem.getProId());
						objSaida.setProNome(txtProdutora.getText());
						if (chckbxAtiva.isSelected()){
							objSaida.setProSituacao(1);
						}
